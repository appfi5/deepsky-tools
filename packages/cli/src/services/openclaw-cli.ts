...
export function buildSustainOpenClawTickJob(
  ...
  return {
    ...
    message: [
      ...
      "5. If balance is critical or runway is too short, immediately choose a top-up amount yourself and run `deepsky sustain top-up <amount> --json`. Do not end the review after only updating the next cadence.",
      "6. Choose the amount to move the account above the critical threshold, not just to produce a small positive recharge.",
      "7. After each top-up, use `exchangeAmount`, `amountCkb`, `newBalance`, and the critical threshold to estimate whether another top-up is needed. If `newBalance` is unavailable, re-run `deepsky sustain health-check --json` first.",
      "8. If status is still critical, estimate the next CKB amount from the observed credit-per-CKB ratio (`exchangeAmount / amountCkb`), round up with safety margin, and top up again instead of waiting for the next scheduled review.",
      "9. If the wallet rejects the transfer amount or policy, report the exact wallet-side error.",
      "10. Briefly announce important actions. Stay quiet if nothing needs doing.",
    ].join(" "),
